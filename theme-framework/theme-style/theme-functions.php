...
function see_my_cv_register_theme_js_scripts() {
	...
	wp_localize_script('see-my-cv-theme-script', 'cmsmasters_theme_script', $cmsmasters_localize_array);
}

add_action('wp_enqueue_scripts', 'see_my_cv_register_theme_js_scripts');



/* Scripts for Admin */
function see_my_cv_theme_admin_scripts() {
	$cmsmasters_option = see_my_cv_get_global_options();
	
	wp_enqueue_script('see-my-cv-theme-settings-toggle', get_template_directory_uri() . '/theme-framework/theme-style' . CMSMASTERS_THEME_STYLE . '/admin/js/theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('see-my-cv-theme-settings-toggle', 'cmsmasters_theme_settings', array( 
		'shortname' => 	'see-my-cv', 
		'header_style' => $cmsmasters_option['see-my-cv' . '_header_styles'] 
	));
}

add_action('admin_enqueue_scripts', 'see_my_cv_theme_admin_scripts');



/* Register Theme Navigations */
register_nav_menus(array( 
    'primary' => 	esc_html__('Primary Navigation', 'see-my-cv'), 
    'footer' => 	esc_html__('Footer Navigation', 'see-my-cv'), 
	'top_line' => 	esc_html__('Top Line Navigation', 'see-my-cv') 
));
...
function see_my_cv_fallback_menu($args) {
	$cmsmasters_option = see_my_cv_get_global_options();
	
	
	echo '<div class="navigation_wrap mid_nav_container">' . "\n" . 
...
function see_my_cv_single_project_layout($cmsmasters_layout) {
	if (
		is_singular('project') || 
		is_singular('profile') 
	) {
...
add_filter('cmsmasters_theme_page_layout_filter', 'see_my_cv_single_project_layout');



/* Search Form */
function see_my_cv_search_form($form) {
	$form = "<div class=\"search_bar_wrap\">
		<form method=\"get\" action=\"" . esc_url(home_url('/')) . "\">
			<p class=\"search_field\">
				<input name=\"s\" placeholder=\"" . esc_attr__('Search...', 'see-my-cv') . "\" value=\"\" type=\"search\" />
			</p>
...
add_filter('get_search_form', 'see_my_cv_search_form');



/* Pagination Prev Text */
function see_my_cv_pagination_prev_text() {
	return esc_html__('previous', 'see-my-cv');
}

add_filter('see_my_cv_pagination_prev_text_filter', 'see_my_cv_pagination_prev_text');



/* Pagination Next Text */
function see_my_cv_pagination_next_text() {
	return esc_html__('next', 'see-my-cv');
}

add_filter('see_my_cv_pagination_next_text_filter', 'see_my_cv_pagination_next_text');
