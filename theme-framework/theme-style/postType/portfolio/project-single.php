...
if (
	$cmsmasters_option['see-my-cv' . '_portfolio_project_like'] || 
	$cmsmasters_option['see-my-cv' . '_portfolio_project_date'] || 
	$cmsmasters_option['see-my-cv' . '_portfolio_project_cat'] || 
	$cmsmasters_option['see-my-cv' . '_portfolio_project_comment'] || 
	$cmsmasters_option['see-my-cv' . '_portfolio_project_author'] || 
	$cmsmasters_option['see-my-cv' . '_portfolio_project_tag'] || 
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_project'); ?>>
	...
	if ($cmsmasters_post_format == 'gallery') {
		...
		see_my_cv_post_type_gallery(get_the_ID(), $cmsmasters_project_images, $cmsmasters_project_columns, 'cmsmasters-project-thumb', 'cmsmasters-project-full-thumb');
		...
		see_my_cv_post_type_video(get_the_ID(), $cmsmasters_project_video_type, $cmsmasters_project_video_link, $cmsmasters_project_video_links, 'cmsmasters-project-full-thumb');
		...
		if ($cmsmasters_project_image_show == 'true' && $cmsmasters_project_images[0] == '') {
			...
			see_my_cv_post_type_slider(get_the_ID(), $cmsmasters_project_images, 'cmsmasters-full-masonry-thumb');
		}
	}
	...
	echo '<div class="project_content' . (($project_sidebar == 'true') ? ' with_sidebar' : '') . '">';
		...
		if ($cmsmasters_project_title == 'true') {
			echo '<header class="cmsmasters_project_header entry-header">';
				see_my_cv_project_title_nolink(get_the_ID(), 'h3');
			echo '</header>';
		}
		...
		echo '<div class="project_sidebar">';
			...
			if ($project_details == 'true') {
				echo '<div class="project_details entry-meta">' . 
					...
					see_my_cv_get_project_likes('post');
					
					see_my_cv_get_project_comments('post');
					
					see_my_cv_get_project_author('post');
					
					see_my_cv_get_project_date('post');
					
					see_my_cv_get_project_category(get_the_ID(), 'pj-categs', 'post');
					
					see_my_cv_get_project_tags(get_the_ID(), 'pj-tags');
					
					see_my_cv_get_project_features('details', $cmsmasters_project_features, false, 'h3', true);
					
					see_my_cv_project_link($cmsmasters_project_link_text, $cmsmasters_project_link_url, $cmsmasters_project_link_target);
					
				echo '</div>';
			}
			
			
			see_my_cv_get_project_features('features', $cmsmasters_project_features_one, $cmsmasters_project_features_one_title, 'h3', true);
			
			see_my_cv_get_project_features('features', $cmsmasters_project_features_two, $cmsmasters_project_features_two_title, 'h3', true);
			
			see_my_cv_get_project_features('features', $cmsmasters_project_features_three, $cmsmasters_project_features_three_title, 'h3', true);
...
if ($cmsmasters_project_sharing_box == 'true') {
	see_my_cv_sharing_box(esc_html__('Share it:', 'see-my-cv'), 'h2');
}


if ($cmsmasters_option['see-my-cv' . '_portfolio_project_nav_box']) {
	$order_cat = (isset($cmsmasters_option['see-my-cv' . '_portfolio_project_nav_order_cat']) ? $cmsmasters_option['see-my-cv' . '_portfolio_project_nav_order_cat'] : false);
	
	see_my_cv_prev_next_posts($order_cat, 'pj-categs');
}
...
if ($cmsmasters_project_more_posts != 'hide') {
	see_my_cv_related( 
		'h2', 
		esc_html__('More projects', 'see-my-cv'), 
		esc_html__('No projects found', 'see-my-cv'), 
		$cmsmasters_project_more_posts, 
		$tgsarray, 
		$cmsmasters_option['see-my-cv' . '_portfolio_more_projects_count'], 
		$cmsmasters_option['see-my-cv' . '_portfolio_more_projects_pause'], 
		'project', 
		'pj-tags' 
	);
}
...
if ($cmsmasters_project_author_box == 'true') {
	see_my_cv_author_box(esc_html__('About author', 'see-my-cv'), 'h2', 'h4');
}
