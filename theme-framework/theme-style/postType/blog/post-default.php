...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_post_default'); ?>>
	...
	if (is_sticky()) { echo '<div class="cmsmasters_post_sticky_head_wrap cmsmasters_theme_icon_sticky">'; }
	
		see_my_cv_post_heading(get_the_ID(), 'h3');
		
		$date ? see_my_cv_get_post_date('page', 'default') : '';
		
	if (is_sticky()) { echo '</div>'; }
	...
	<div class="cmsmasters_post_cont">
		...
		if ($cmsmasters_post_format == 'image') {
			...
			see_my_cv_post_type_slider(get_the_ID(), $cmsmasters_post_images, 'post-thumbnail');
		} elseif ($cmsmasters_post_format == '' && !post_password_required() && has_post_thumbnail()) {
			see_my_cv_thumb(get_the_ID(), 'post-thumbnail', true, false, false, false, false, true, false);
