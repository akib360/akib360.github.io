...
function see_my_cv_prev_next_posts($order_cat = false, $tax_name = 'category') {
	...
	if ($published_posts > 1) {
		...
		previous_post_link('<span class="cmsmasters_prev_post">%link<span class="cmsmasters_prev_arrow"><span></span></span><span class="sub">'. esc_html__('previous', 'see-my-cv') .'</span></span>', '%title', $order_cat, '', $tax_name);
		
		next_post_link('<span class="cmsmasters_next_post">%link<span class="cmsmasters_next_arrow"><span></span></span><span class="sub">'. esc_html__('next', 'see-my-cv') .'</span></span>', '%title', $order_cat, '', $tax_name);
...
function see_my_cv_sharing_box($title_box = false, $tag = 'h3') {
...
function see_my_cv_author_box($title_box = false, $tag = 'h3', $author_tag = 'h4') {
	...
	if ($user_description) {
		...
		echo '<figure class="about_author_avatar">' . 
			get_avatar($user_email, 150, get_option('avatar_default')) . 
		'</figure>' . 
		...
		if ($user_url) {
			echo '<a href="' . esc_url($user_url) . '" title="' . esc_attr(get_the_author()) . ' ' . esc_attr__('website', 'see-my-cv') . '" target="_blank">' . esc_html($user_url) . '</a>';
		}
...
function see_my_cv_related($tag = 'h3', $title = '', $no_title = '', $box_type = false, $tgsarray = null, $items_number = 5, $pause_time = 5, $type = 'post', $taxonomy = null) {
	if ( 
		...
		if ($r_query->have_posts()) {
			echo "<aside class=\"cmsmasters_single_slider\">" . 
				"<" . esc_html($tag) . " class=\"cmsmasters_single_slider_title\">" . 
					($title != '' ? esc_html($title) : esc_html__('More items', 'see-my-cv')) . 
				"</" . esc_html($tag) . ">" . 
						...
						while ($r_query->have_posts()) : $r_query->the_post();
							echo "<div class=\"cmsmasters_owl_slider_item cmsmasters_single_slider_item\">
								<div class=\"cmsmasters_single_slider_item_outer\">";
									...
									see_my_cv_thumb(get_the_ID(), 'cmsmasters-project-thumb', true, false, true, false, true, true, false, false, false, 'cmsmasters_theme_icon_image');
...
function see_my_cv_author_avatar($template_type = 'page') {
...
function see_my_cv_get_pings($id, $tag = 'h3') {
	$out = '';
	...
	if (sizeof($pings) > 0) {
		$out .= '<aside class="cmsmasters_pings_list">' . "\n" .
			'<' . esc_html($tag) . '>' . esc_html__('Trackbacks and Pingbacks', 'see-my-cv') . '</' . esc_html($tag) . '>' . "\n" .
